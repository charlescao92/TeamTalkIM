...
build() {
    ...
    for i in base slog  route_server etcd_msg_server http_msg_server file_server push_server tools db_proxy_server msfs etcd_login_server ; do     
        cd $CURPWD/$i
        cmake .
        make
        if [ $? -eq 0 ]; then
    ...
    cp etcd_login_server/login_server ../run/login_server/

    cp route_server/route_server ../run/route_server/

    cp etcd_msg_server/msg_server ../run/msg_server/
    ...
    cp etcd_login_server/login_server ../$build_version/login_server/
    cp etcd_login_server/loginserver.conf ../$build_version/login_server/

    cp route_server/route_server ../$build_version/route_server/
    cp route_server/routeserver.conf ../$build_version/route_server/

    cp etcd_msg_server/msg_server ../$build_version/msg_server/
    cp etcd_msg_server/msgserver.conf ../$build_version/msg_server/
