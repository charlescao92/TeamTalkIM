...
void server_register_timer_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
    ...
    Json::Value root;
    root["host_ip"] = g_reg_info.host_ip;
    ...
    if(resp->err) {
        ...
        log_error("cetcd_set error :%d, %s (%s)\n", resp->err->ecode, resp->err->message, resp->err->cause);
...
int init_server_register(const LoginServerRegInfo *reg_info)
{
    ...
    if(resp->err) {
        printf("cetcd_mkdir error :%d, %s (%s)\n", resp->err->ecode, resp->err->message, resp->err->cause);
        log_warn("cetcd_mkdir error :%d, %s (%s)\n", resp->err->ecode, resp->err->message, resp->err->cause);
        
        // cetcd_mkdir error :105, Key already exists (/login_servers)
        if (resp->err->ecode != 105) {
            return -1;
        }
    }
    cetcd_response_print(resp);
